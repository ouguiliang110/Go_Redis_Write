...
func (dict *SyncDict) Get(key string) (val interface{}, exists bool) { //取Get
...
func (dict *SyncDict) ForEach(consumer Consumer) {
	dict.m.Range(func(key, value interface{}) bool {
		consumer(key.(string), value) //不断执行这个函数
		return true
	})
}
...
func (dict *SyncDict) RandomKeys(limit int) []string {
	...
	for i := 0; i < limit; i++ {
		dict.m.Range(func(key, value interface{}) bool { //作用一次，进入下一个for
			result[i] = key.(string)
			return false
		})
	}
